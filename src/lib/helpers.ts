import { path, fse, _ } from 'tnp-core/src';
...
export class HelpersSwagger {
  ...
  static prepareModel(
    swg: Models.Swagger.SwaggerModel,
    indexSwg: number,
  ...
  static get groupFromJSON() {
    ...
    return {
      ...
      folderName(swg: Models.Swagger.SwaggerModel) {
        ...
        return _.upperFirst(
          _.camelCase(basePath.trim() === '' ? `json${index}` : basePath),
        );
      },
    };
  }
  ...
  static getObjectDefinition(
    ...
    let obj: Models.Swagger.SwaggerDef = <Models.Swagger.SwaggerDef>(
      _.get(swg, ref)
    );
